...
# Public URL used for "verify" links the bot prints after /load and /done.
# Falls back to Railway's auto-injected RAILWAY_PUBLIC_DOMAIN when PLUXO_PUBLIC_URL is unset.
PLUXO_PUBLIC_URL = (
    os.environ.get("PLUXO_PUBLIC_URL")
    or os.environ.get("RAILWAY_PUBLIC_DOMAIN")
    or ""
).strip().rstrip("/")
if PLUXO_PUBLIC_URL and not PLUXO_PUBLIC_URL.startswith(("http://", "https://")):
    PLUXO_PUBLIC_URL = "https://" + PLUXO_PUBLIC_URL
...
async def tg_load(update, context) -> None:
    ...
    if cmd_pair:
        ...
        verify_line = ""
        if PLUXO_PUBLIC_URL:
            vurl = f"{PLUXO_PUBLIC_URL}/api/debug/state?user={u}"
            verify_line = f'\n\nVerify: <a href="{html.escape(vurl, quote=True)}">{html.escape(vurl)}</a>'
        await msg.reply_text(
            f"✅ Loaded <b>${amt:.2f}</b>{extra} → <b>{html.escape(u)}</b>\n"
            f"New balance: <b>${nb:.2f}</b>" + verify_line,
            parse_mode="HTML",
            disable_web_page_preview=True,
        )
        return
...
async def _tg_stock_reply_added(
    ...
    with state_lock:
        live_count = len(state.get("stock") or [])
    extra_lines = []
    if PLUXO_PUBLIC_URL:
        verify_url = f"{PLUXO_PUBLIC_URL}/api/products"
        extra_lines.append(
            f'\nVerify backend has the cards: <a href="{html.escape(verify_url, quote=True)}">{html.escape(verify_url)}</a>'
        )
    await msg.reply_text(
        f"✅ Added <b>{added}</b> card(s) at <code>${price:.2f}</code> → <b>{lbl}</b> "
        f"(country <code>{html.escape(country_code)}</code>).\n"
        f"Total shop stock on this backend now: <b>{live_count}</b>." + "".join(extra_lines) +
        "\n\nIf the site shows old stock: hard refresh (Ctrl+Shift+R) and make sure "
        "<code>API URL</code> in your browser console points at this Railway deploy.\n\n"
...
# Startup diagnostics: dump state file path + counts so Railway logs show whether
# this process actually sees the bot's writes.
try:
    _start_stt = STATE_PATH.stat() if STATE_PATH.exists() else None
    print(
        "[pluxo] STARTUP "
        f"pid={os.getpid()} state_path={STATE_PATH} "
        f"exists={STATE_PATH.exists()} "
        f"size={_start_stt.st_size if _start_stt else 0} "
        f"users={len(state.get('users') or {})} "
        f"stock={len(state.get('stock') or [])} "
        f"topups={len(state.get('crypto_topups') or {})}",
        flush=True,
    )
except Exception as _start_exc:
    print(f"[pluxo] startup diag failed: {_start_exc!r}", flush=True)
