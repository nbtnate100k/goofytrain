...
async def tg_topup_callback(update, context) -> None:
    ...
    for attempt in range(10):
        if attempt > 0:
            await asyncio.sleep(0.08 + attempt * 0.04)
            _reload_crypto_topups_from_disk()
        with state_lock:
            pend = _find_crypto_topup_row_unlocked(pid)
            if not pend:
                err = "missing"
                continue
    ...
    if err == "missing":
        await q.answer(
            f"Request not found (id {pid}). Try again in a few seconds — "
            "if it keeps failing, ensure the site and Telegram bot share the same data/state.json.",
            show_alert=True,
        )
        return
    if err == "done":
